...
describe AvetmissData::Stores::V7::Course do
  context 'imports the NAT00030 file' do
    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V7::Course.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V7::Course.file_name).to eq('NAT00030') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v7/NAT00030.txt').first }
      subject { AvetmissData::Stores::V7::Course.from_line(line) }
    ...
    context 'invalid record' do
      let!(:line) { File.open('spec/fixtures/nat_files/v7/NAT00030_identifier.txt').first }
      subject { AvetmissData::Stores::V7::Course.from_line(line) }
