...
describe AvetmissData::Stores::V7::UnitOfCompetency do
  context 'imports the NAT00060 file' do

    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V7::UnitOfCompetency.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V7::UnitOfCompetency.file_name).to eq('NAT00060') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v7/NAT00060.txt').first }
      subject { AvetmissData::Stores::V7::UnitOfCompetency.from_line(line) }
