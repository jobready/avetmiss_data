...
describe AvetmissData::Stores::V8::Client do
  describe 'NAT File' do
    specify { expect(AvetmissData::Stores::V8::Client.file_name).not_to be_blank }
    specify { expect(AvetmissData::Stores::V8::Client.file_name).to eq('NAT00080') }
  end

  describe 'v8 NAT00080 record' do
    let!(:line) { File.open(file_path).first }
    subject { AvetmissData::Stores::V8::Client.from_line(line) }
    ...
    context 'when the record has extras' do
