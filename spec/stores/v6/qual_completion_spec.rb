...
describe AvetmissData::Stores::V6::QualCompletion do
  context 'imports the NAT00130 file' do

    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V6::QualCompletion.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V6::QualCompletion.file_name).to eq('NAT00130') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00130.txt').first }
      subject { AvetmissData::Stores::V6::QualCompletion.from_line(line) }
