...
describe AvetmissData::Stores::V6::Enrolment do
  context 'imports the NAT00120 file' do

    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V6::Enrolment.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V6::Enrolment.file_name).to eq('NAT00120') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00120.txt').first }
      subject { AvetmissData::Stores::V6::Enrolment.from_line(line) }
