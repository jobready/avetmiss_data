...
describe AvetmissData::Stores::V6::RtoDeliveryLocation do
  context 'imports the NAT00020 file' do

    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V6::RtoDeliveryLocation.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V6::RtoDeliveryLocation.file_name).to eq('NAT00020') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00020.txt').first }
      subject { AvetmissData::Stores::V6::RtoDeliveryLocation.from_line(line) }
