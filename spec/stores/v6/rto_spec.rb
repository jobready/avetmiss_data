...
describe AvetmissData::Stores::V6::Rto do
  context 'imports the NAT00010 file' do

    context 'NAT File' do
      specify { expect(AvetmissData::Stores::V6::Rto.file_name).not_to be_blank }
      specify { expect(AvetmissData::Stores::V6::Rto.file_name).to eq('NAT00010') }
    end

    context 'NAT Record Parse' do
      let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00010.txt').first }
      subject { AvetmissData::Stores::V6::Rto.from_line(line) }
  ...
  context 'invalid record' do
    let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00010_invalid.txt').first }
    subject { AvetmissData::Stores::V6::Rto.from_line(line) }
    specify { expect(subject).not_to be_blank }
  end

  context 'record with data collection fields' do
    let!(:line) { File.open('spec/fixtures/nat_files/v6/NAT00010_extrafields.txt').first }
    subject { AvetmissData::Stores::V6::Rto.from_line(line) }
    specify { expect(subject).not_to be_blank }
    specify { expect(subject.extras).not_to be_blank }
  end
